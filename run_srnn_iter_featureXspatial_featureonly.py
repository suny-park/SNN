...
# s_stim_amps = np.arange(8,15) # strength of bottom-up stimulus
# r_stim_amps_F = np.arange(10) # strength of top-down gain for feature attention
# r_stim_amps_S = np.arange(10) # strength of top-down gain for spatial attention
r_stim_amps_F = [0,2,4,6,8] # strength of top-down gain for feature attention
r_stim_amps_S = [0] # strength of top-down gain for spatial attention
...
N_trials_attention = 10 # we loop over 16 stimulus pairs for attention task, so can go with lower trial numbers
...
# N_reps =  # initialize networks across reps - this is also the random seed value
# reps=thisrep
...
# for rep in np.arange(N_reps):
    # for rep in reps:
    t = time.time()
    print('running rep'+str(rep)+ ' out of '+str(N_reps)+' total')
    #-------------------------
...
# for ss_cnt, StimStrength in enumerate(stim_strengths): 
    ...
    fn_decoding = 'results/F_test_inh-'+str(S_rec_w_kappa_inh)+'_seed-'+str(rep)+'.npz'
    if saveFile:
