...
# fully random connection between layers
# define top-down gain set from weights to each sub-network
...
# s_stim_amps = np.arange(0,10) # strength of bottom-up stimulus
# r_stim_amps = np.arange(0,10) # strength of top-down gain

# s_stim_amps = np.linspace(0,9.5,num=20)

s_stim_amps = np.arange(0,20)
# r_stim_amps = [0,2,4,6,8]
r_stim_amps = [10,12,14,16,18]
# r_stim_amps = [0,1]

r_stim_ratios = [0.2] # proportion of second layer units to apply top-down gain
r_conn_kappas = [0] # spread of between-layer connections. higher value means more structure -- fully random for spatial

N_trials_sensory = 0 # separate values for sensory and attention task trials
N_trials_attention = 50 # want this to be comparable to the F version...
...
from srnn_iter_spatial import * # Make sure this is the one we want!!
...
# for ss_cnt, StimStrength in enumerate(stim_strengths): 
    ...
    init_params = {'rndSeed' : rndSeed, 'N_trials_sensory' : N_trials_sensory, 'N_trials_attention' : N_trials_attention, 
                  ...
                  'StimExposeTime' : StimExposeTime, 'AttnTime' : AttnTime,'PreAttTime' : PreAttTime,
                  ...
                  's_stim_amps' : s_stim_amps, 'r_stim_amps' : r_stim_amps, 'r_stim_ratios' : r_stim_ratios,
    ...
    S_fr_avg_loc, label_stim_loc, label_pool_loc, label_trial_loc, \
        S_fr_avg_main, S_fr_avg_main_base, label_stim_main, label_trial_main, \
    ...
    fn_decoding = 'results/S_higher-Rstim_seed-'+str(rep)+'.npz'
    if saveFile:
        np.savez(fn_decoding, S_fr_avg_loc=S_fr_avg_loc, label_stim_loc=label_stim_loc, \
                 label_pool_loc=label_pool_loc, label_trial_loc=label_trial_loc, \
                     S_fr_avg_main= S_fr_avg_main, S_fr_avg_main_base=S_fr_avg_main_base, label_stim_main= label_stim_main, \
                         label_trial_main=label_trial_main, N_stim_loc=N_stim_loc, \
                             r_stim_amps=r_stim_amps, r_stim_ratios=r_stim_ratios, s_stim_amps=s_stim_amps, \
