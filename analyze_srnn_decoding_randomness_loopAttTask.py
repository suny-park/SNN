...
fileName = 'FBA_decoding_shiftStep32_numMatched_randomness'+str(rand_kappa)
...
# R_stim = 0 # one r_stim_ratio for now
 ...
 # TODO: plot likelihood estimates for kappa = 0 and kappa = 0.4
# fr_att_abs[tc,:,tw,r_cnt,r_prop]
mag_att = np.full((S_N_pools,len(r_stim_amps)),np.nan)
R_ratio = 0 # one r_stim_ratio for now
for r_cnt, R_stim in enumerate(r_stim_amps):    
    
    mag_att[:,r_cnt] = np.mean(np.mean(fr_att_abs[:,:,-5:,r_cnt,R_ratio],axis=0),axis=1) # average over all trials and then the last 5 timewindows
    
mag_att2 = np.full((2,len(r_stim_amps)),np.nan)
mag_att2[0,:] = mag_att[0,:]
mag_att2[1,:] = np.mean(mag_att[1:,:],axis=0)
...
plt.show()
    # for thispool in np.arange(S_N_pools):
        ...
        # mag_att[thispool] = np.mean(np.squeeze(np.mean(fr_att_abs[:,thispool,-5:,r_cnt,R_ratio],axis=0)),axis=0) # the magnitude of likelihood for the attended feature for the last timewindow

#%%
rK = [0,0.1,0.2,0.3,0.4]
x = np.linspace(0, (pi * 2)-((pi * 2) / S_N_neuron), S_N_neuron)
all_func = np.full((S_N_neuron,len(rK)),np.nan)
for i,rk in enumerate(rK):
    circ_norm_func = np.exp(rk * np.cos(x-pi))/(2 * pi * i0(rk)) 
