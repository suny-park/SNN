...
class rand_attn_inh:
    ...
    def __init__(self, params):
        ...
        self.PingStimScale = params.get('PingStimScale', 0)
        self.PingType = params.get('PingType', 1)                                 # 1 is IID white noise for the ping (distractor), 0 is a structured step function
        self.PingStimPoolsOrAllPools = params.get('PingStimPoolsOrAllPools', 0) # if 0 then only ping pools that got a stim, else ping all pools
        self.PingStimOnset = params.get('PingStimOnset', 0.2 * second)          # how long after stim offset to present ping? 
        self.PingExposeTime = params.get('PingExposeTime', 0.5 * second)        # how long to present ping?        
        self.PingNumPulses = params.get('PingNumPulses', 1)                     # how many times to changae the ping stim on each trial
    ...
    def define_ping_stims(self, StimVals, AttnStim):
        """
        Define the 'pings', or distractors, to probe the system


        Returns
        -------
        PingStims : numpy array of size = [#pools, #neuron], but flattened

        """
        #------------------------------------------------ 
        # define ping stims that are scaled IID noise stims
        #------------------------------------------------
        
        # if generating a square-wave structured ping stim
        if self.PingType == 0:
        
            # if pinging all pools EXCEPT the stimulated pool
            if self.PingStimPoolsOrAllPools == 2:
                PingStims = np.zeros((self.S_N_pools, self.S_N_neuron))
                for sp in np.arange(self.S_N_pools):
                    if sp not in self.S_stim_pools:
                        step = np.zeros(self.S_N_neuron)
                        step[0:np.random.randint(0,self.S_N_neuron)] = 1
                        roll_step = np.roll(step, np.random.randint(0,self.S_N_neuron))
                        PingStims[sp,:] = self.PingStimScale * roll_step
                        
            # if pinging all pools
            elif self.PingStimPoolsOrAllPools == 1:
                PingStims = np.zeros((self.S_N_pools, self.S_N_neuron))
                for sp in np.arange(self.S_N_pools):
                    step = np.zeros(self.S_N_neuron)
                    step[0:np.random.randint(0,self.S_N_neuron)] = 1
            ...
            # otherwise just ping stimulated pools
            elif self.PingStimPoolsOrAllPools == 0: 
                PingStims = np.zeros((self.S_N_pools, self.S_N_neuron))
                for sp in np.arange(self.S_N_pools):
        ...
        # if generating a white noise (IID) ping stim
        elif self.PingType == 1:
            # if pinging all pools EXCEPT the stimulated pool
            if self.PingStimPoolsOrAllPools == 2:
                PingStims = np.zeros(self.S_N_pools, self.S_N_neuron) 
                for sp in np.arange(self.S_N_pools):
                    if sp not in self.S_stim_pools:
                        PingStims[sp*self.S_N_neuron:sp*self.S_N_neuron+self.S_N_num_stimvals] = self.PingStimScale * np.random.rand(self.S_N_pools, self.S_N_neuron)            
            
            # if pinging all pools including the stimulated pool
            elif self.PingStimPoolsOrAllPools == 1: 
                PingStims = self.PingStimScale * np.random.rand(self.S_N_pools, self.S_N_neuron)
            ...
            # otherwise just ping stimulated pools
                ...
                for sp in np.arange(self.S_N_pools):
                    if ~np.isnan(StimVals[sp]):
                        PingStims[sp,:] = self.PingStimScale * np.random.rand(self.S_N_neuron) 
        
        # if generating structured ping with von mises
        # that is just like the actual stim...
        elif self.PingType == 2:
            #------------------------------------------------ 
            ...
            #------------------------------------------------ 
            tmp_ping = np.exp(self.kappa * np.cos(x-pi))/(2 * pi * i0(self.kappa)) 
            ...
            tmp_ping[:int(self.S_N_neuron/2)-cutoff] = 0
            tmp_ping[int(self.S_N_neuron/2)+cutoff:] = 0
        
            #------------------------------------------------         
            # normalize to amp == 1
            #------------------------------------------------ 
            tmp_ping /= np.max(tmp_ping)         
        
            # if pinging all pools EXCEPT for the stimulated pool
            if self.PingStimPoolsOrAllPools == 2:
                PingStims = np.zeros((self.S_N_pools, self.S_N_neuron))
                for sp in np.arange(self.S_N_pools):
                    if sp not in self.S_stim_pools:
                        roll_ping = np.roll(tmp_ping, np.random.randint(0,self.S_N_neuron))
                        PingStims[sp,:] = self.PingStimScale * roll_ping 
                    else:
                        PingStims[sp,:] = AttnStim[sp * self.S_N_neuron : sp * self.S_N_neuron + self.S_N_neuron]
            ...
            # if just pinging stimulated pool(s)
            elif self.PingStimPoolsOrAllPools == 0: 
                PingStims = np.zeros((self.S_N_pools, self.S_N_neuron))
                for sp in np.arange(self.S_N_pools):
                    if ~np.isnan(StimVals[sp]):
    ...
    def run_sim_rand(self):
        ...
        for t in np.arange(self.N_trials):
            ...
            print('Running trial:', t, 'Rand:', self.rand_top_down, 'Number To Stim:', self.R_NumNeuronsToStim)
            ...
            #------------------------------------------------
            # if pinging then wait the stim-to-ping ISI
            # then apply the ping, then wait the rest of rest of the 
            # WM delay period
            #------------------------------------------------
            if self.PingStimScale > 0: # get rid of this - ideally have one run statement
                
                # wait until ping onset time
                run(self.PingStimOnset)
               
                for num_pings in np.arange(self.PingNumPulses):
                    #------------------------------------------------
                    # generate ping stims - these are also rand
                    # generated and scaled by PingStimScale
                    #------------------------------------------------
                    ping_stims = self.define_ping_stims(stim_vals[t,:],stims)
                    
                    # assign the ping stims
                    S_pools.Stim = ping_stims
                
                    # run for ping duration
                    run(self.PingExposeTime / self.PingNumPulses)
                
                # # then reset to baseline
                # S_pools.Stim = base_in
                
                # then wait the remaining part of the WM delay period
                run(self.AttnTime - self.PingStimOnset - self.PingExposeTime)
            
            #------------------------------------------------
            # if not pinging then just wait the WM delay period
            #------------------------------------------------
            else:
                run(self.AttnTime)
        ...
        return fr_angle, fr_abs, fr_mem_abs, stim_vals, stim_pools, S_fr, R_fr
