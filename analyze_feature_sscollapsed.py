...
# if doPlot:
#     # Plot pdf for different kappas
#     x = np.linspace(0, (pi * 2)-((pi * 2) / S_N_neuron), S_N_neuron)
#     all_func = np.full((S_N_neuron,len(rK)),np.nan)
#     for i,kappas in enumerate(kappas):
#         circ_norm_func = np.exp(kappas * np.cos(x-pi))/(2 * pi * i0(kappas)) 
#         circ_norm_func /= np.sum(circ_norm_func) # normalize to amp = 1
#         all_func[:,i] = circ_norm_func

#     plt.figure(figsize=(4,4))
#     plt.plot(all_func)
#     # plt.title('kappa = '+str(rK))
#     plt.xlabel('First layer sub-network neurons')
#     plt.ylabel('Probability')
#     plt.legend(kappas,frameon=False)
#     plt.ylim(0,0.005)
#     plt.show()
...
# rand_kappa = 0
    ...
    dataFile = 'F_kappa-'+str(rand_kappa)+'_seed-1.npz'
    ...
    dataFile = 'F_higher-Rstim_kappa-'+str(rand_kappa)+'_seed-1.npz'    
    data = np.load(dataFile)    
    ...
    for rep_cnt, rep in enumerate(reps):
        dataFile = 'F_kappa-'+str(rand_kappa)+'_seed-'+str(rep)+'.npz'
        print('Loading '+dataFile)
        data = np.load(dataFile)
    ...
    for r_cnt, R_stim in enumerate(r_stim_amps):
        for rep_cnt, rep in enumerate(reps):
            ...
            # combine data for higher attention gain sims
            dataFile = 'F_higher-Rstim_kappa-'+str(rand_kappa)+'_seed-'+str(rep)+'.npz'    
            data = np.load(dataFile)    
            r_stim_amps = np.append(r_stim_amps,data['r_stim_amps'])
            print(r_stim_amps)
            S_fr_avg_main = np.concatenate((S_fr_avg_main,data['S_fr_avg_main']),axis=2)
        ...
        # sig2 = np.full((S_N_pools,2),np.nan) # to bucket circ corr coef and p_vals
            for thispool in np.arange(S_N_pools):
                ...
                # for ss_cnt in np.arange(len(stim_strengths)):
                    ...
                    MAE2[r_cnt,thispool,rep_cnt,s_cnt] = \
                        circ_MAE(tsty[this_stim_trial_idx]/180*pi, \
                                 pred_deg[this_stim_trial_idx]/180*pi)*180/pi
        if doPlot:
            plt.figure(figsize=(4,4))
    ...
    datdic = {'decoding_sen2sen':decoding1, 'MAE_sen2sen':MAE1, 'decoding_sen2att':decoding2, 'MAE_sen2att':MAE2,'decoding_att2att':dec_pool,'decoding_att2att_xpool':dec_x_pool,
    ...
    savemat('result_F_kappa-'+str(rand_kappa)+'.mat',datdic)
    print('saved result_F_kappa-'+str(rand_kappa)+'.mat')
